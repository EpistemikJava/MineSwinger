...
class MineField extends JLabel implements MouseListener
{
  ...
  * ========================================================================================= */
  ...
  /**
   * CONSTRUCTOR <br> called ONLY by <code>MineSwinger.createMineField()</code>
  ...
  private void reset( int len )
  {
    ...
    System.out.println( "totalMines = " + totalMines ); //$NON-NLS-1$
    System.out.println( "density = " + density ); //$NON-NLS-1$
  ...
  protected void clearQmarks()
  {
    ...
    for( i=0; i<fieldLength; i++ )
      for( j=0; j<fieldLength; j++ )
      {
        ...
        {
          if( DEBUG_LEVEL > 1 )
            System.out.println( myname() + Msgs.str("Qmarks.clear") //$NON-NLS-1$
                                + "Square[" + i + "][" + j //$NON-NLS-1$ //$NON-NLS-2$
                                + "] " + Msgs.str("Qmark.has") ); //$NON-NLS-1$ //$NON-NLS-2$
  ...
  protected void layMines()
  {
    ...
    System.out.println( "hiddenMines = " + hiddenMines ); //$NON-NLS-1$
  ...
  public void mousePressed( final MouseEvent evt )
  {
    ...
    if( DEBUG_LEVEL > 2 )
      System.out.println( myname() + Msgs.str("mouse.press") + "Square[" + x //$NON-NLS-1$ //$NON-NLS-2$
    ...
    if( DEBUG_LEVEL > 1 )
      System.out.println( myname() + Msgs.str("mouse.press") + InputEvent.getModifiersExText(pressed) ); //$NON-NLS-1$
          ...
          for( j=top; j <= bottom; j++ )
          {
            if( mine2dArray[i][j].hasFlag() )
            {
              ...
              if( !mine2dArray[i][j].hasMine() )
              {
                ...
                setGameText( Msgs.str("badclear.info") ); //$NON-NLS-1$
                paintLeft = i ;
                paintTop = j ;
                badClear = true ;
                return ;
     ...
     * button 3 = set flag for normal or qmark, remove flag is one is present  */
    {
      ...
      if( (pressed & InputEvent.BUTTON1_DOWN_MASK) == InputEvent.BUTTON1_DOWN_MASK )
      {
        if( activeSqr.hasFlag() || activeSqr.hasQmark() )
          return ;
      ...
      if( (pressed & InputEvent.BUTTON2_DOWN_MASK) == InputEvent.BUTTON2_DOWN_MASK )
      {
        ...
        if( activeSqr.hasQmark() )
        {
          ...
          { // add qmark if they are active
            if( gameview.qMarksAreOn() )
  ...
  public void mouseReleased( final MouseEvent evt )
  {
    ...
    if( DEBUG_LEVEL > 1 )
      System.out.println( myname() + Msgs.str("mouse.release") //$NON-NLS-1$
    ...
    if( destroyed || badClear )
    {
      ...
      if( DEBUG_LEVEL > 2 )
        System.out.println( myname() + Msgs.str("mouse.release") //$NON-NLS-1$
                            + "destx = " + destx + " ; " //$NON-NLS-1$ //$NON-NLS-2$
                            + "desty = " + desty ); //$NON-NLS-1$
      
      if( destroyed ) // activate the explosion
      {
        listener.init();
        setGameText( Msgs.str("explode.info") ); //$NON-NLS-1$
  ...
  {
    if( DEBUG_LEVEL > 1 )
      System.out.println( myname() + ": paintArea() > "         //$NON-NLS-1$ 
                          + Msgs.str("Left")   + paintLeft      //$NON-NLS-1$
                          + Msgs.str("Right")  + paintRight     //$NON-NLS-1$
                          + Msgs.str("Top")    + paintTop       //$NON-NLS-1$
                          + Msgs.str("Bottom") + paintBottom ); //$NON-NLS-1$
  ...
  {
    if( DEBUG_LEVEL > 1 )
      System.out.println( myname() + ": paintComponent() > "    //$NON-NLS-1$ 
                          + Msgs.str("Left")   + paintLeft      //$NON-NLS-1$
                          + Msgs.str("Right")  + paintRight     //$NON-NLS-1$
                          + Msgs.str("Top")    + paintTop       //$NON-NLS-1$
                          + Msgs.str("Bottom") + paintBottom ); //$NON-NLS-1$
   ...
   * i.e. different numbers of adjacent mines, revealed or not, flag, blow-up, etc<br>
  ...
  void reveal( final int xc, final int yc, final int tp, Graphics page )
  {
    ...
    if( DEBUG_LEVEL > 2 )
      System.out.println( myname() + ": reveal() > xc = " + xc     //$NON-NLS-1$
                          + " & yc = " + yc + " & type = " + tp ); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    switch( $type )
    {
      ...
      case FLAG:
                ...
                page.drawString( "*", xc + 3*$sl/8, yc + 7*$sl/8 ); //$NON-NLS-1$
                break ;
    
      case QMARK:
                drawSquare( xc, yc, page );
                page.setColor( COLOR_DARKEST );
                page.drawString( "?", xc + $sl/4, yc + 3*$sl/4 ); //$NON-NLS-1$
                break ;
      ...
      case BADCLEAR:
                ...
                page.drawString( "X", xc + $sl/4, yc + 3*$sl/4 ); //$NON-NLS-1$
                break ;
    
      case FATAL: // animated explosion if user clicked on a mine
      {
        if( DEBUG_LEVEL > 1 )
          System.out.println( "case FATAL" ); //$NON-NLS-1$
  ...
  * ========================================================================================= */
   ...
   */
   class Square
   {
     private boolean hasMine = false, isRevealed = false,
   ...
   class ExplodeListener implements ActionListener
   {
     ...
     public void actionPerformed( final ActionEvent ae )
     {
       if( ae.getSource() == exploder )
       {
         ...
         if( DEBUG_LEVEL > 0 )
           System.out.println( myname() + ": Event index = " + index ); //$NON-NLS-1$
         
         repaint();
       }
       else
           System.err.println( myname() + ": Event source INVALID!" ); //$NON-NLS-1$
